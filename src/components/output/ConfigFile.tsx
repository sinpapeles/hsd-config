...
const ConfigFile: React.FC = () => {
  ...
  const fileContent = useMemo(() => {
    ...
    if (config.indexTX) {
      args.push(`index-tx=true`);
    }

    if (config.indexAddress) {
      args.push(`index-address=true`);
    }
    ...
    if (config.persistMempool) {
      args.push(`persist-mempool=true`);
    }

    if (config.selfish) {
      args.push(`selfish=true`);
    }
    ...
    if (config.bip37) {
      args.push(`bip37=true`);
    }

    if (config.listen) {
      args.push(`listen=true`);
    }
